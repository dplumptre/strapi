...
import { useFocusInputField, useNotification } from '@strapi/helper-plugin';
...
interface Relation extends Pick<Contracts.Relations.RelationResult, 'id' | 'publishedAt'> {
  href: string;
  label: string;
}
...
const RelationsField = React.forwardRef<HTMLDivElement, RelationsFieldProps>(
  ({ disabled, label, ...props }, ref) => {
    ...
    const relations = [...data.results, ...(field.value?.connect ?? [])].map((relation) => ({
      ...relation,
      label: props.mainField && relation[props.mainField] ? relation[props.mainField] : relation.id,
      // @ts-expect-error – targetModel does exist on the attribute. But it's not typed.
      href: `/content-manager/${COLLECTION_TYPES}/${props.attribute.targetModel}/${relation.id}`,
    }));
  ...
  React.useEffect(() => {
    searchForTrigger({
      ...
      params: {
        ...
        id: id ?? '',
        pageSize: 10,
        idsToInclude: field.value?.disconnect?.map((rel) => rel.id) ?? [],
        idsToOmit: field.value?.connect?.map((rel) => rel.id) ?? [],
        ...searchParams,
      },
    });
...
const ListItem = ({ data, index, style }: ListItemProps) => {
  ...
  const { href, id, label, publishedAt } = relations[index];
